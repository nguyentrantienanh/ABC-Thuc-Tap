...
export class RefreshTokensController {
  ...
  async refresh(@Req() req: Request, @Res({ passthrough: true }) response: ExpressResponse) {
    const user = req.user as User;
    const ip = req.ip as string;
    const ua = req.headers['user-agent'] || '';
    const refreshToken = req.cookies['refreshToken'];

    const resp = await this.refreshTokensService.refresh(user.id, refreshToken, ip, ua);
  ...
  async revoke(@Req() req: Request) {
    ...
    return this.refreshTokensService.revoke(refreshToken, ip, ua);
  }
}
