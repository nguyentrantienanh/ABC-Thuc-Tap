...
type SubMenuPagesProps = {
  type: 'dropdown' | 'list';
  onNavigate?: () => void;
};

const SubMenuPages: FC<SubMenuPagesProps> = ({ type, onNavigate }) => {
  ...
  return (
    <div>
      <NavLink
        ...
        onClick={handleClick}
      >
        <p className={classNames('whitespace-nowrap', className)}>{t('sidebar_menu_pages_create_new')}</p>
      </NavLink>
    </div>
  );
};
